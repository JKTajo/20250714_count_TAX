...
def process_ecount_file(df: pd.DataFrame, is_free_tax: bool = False) -> pd.DataFrame:
    ...
    #groupby를 이용해서 day_1, item_1, standard_1, quality_1, unit_price_1, price_1, VAT_1, note_1까지 하나로 묶자.
     df_group1 = df_final.groupby('day_1', 'item_1', 'standard_1', 'quality_1', 'unit_price_1', 'VAT_1', 'note_1')
     df_group2 = df_final.groupby('day_2', 'item_2', 'standard_2', 'quality_2', 'unit_price_2', 'VAT_2', 'note_2')
     df_group3 = df_final.groupby('day_3', 'item_3', 'standard_3', 'quality_3', 'unit_price_3', 'VAT_3', 'note_3')
     for df_group1,df_group2,df_group3 in range(df_final.shape[0]):
       #마지막 열제외 열잡기
       for c in range(df_final.shape[1] - 1):
        if df_final.iloc[df_group1,df_group2,df_group3,c] ==0:
          df_final.iloc[df_group1,df_group2,df_group3,c] = df_final.iloc[df_group1,df_group2,df_group3,c+1]
          df_final.iloc[df_group1,df_group2,df_group3,c+1] = 0
    return df_final
    while True:
